...
export function SliderInput({
  ...
  const [typedValue, setTypedValue] = useState(value.toString());
  ...
  return (
    <div id={id} className={`w-full flex flex-col gap-2.5 ${className}`}>
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-semibold text-[#7A8C6E]">{min.toLocaleString()} {unit}</span>
        <div className="inline-flex items-center rounded-xl bg-[#EDE8DC] p-1 border border-[#DDD8CD]/80 shadow-xs focus-within:ring-2 focus-within:ring-[#1A4D2E]/30">
          <button type="button" onClick={() => handleStep(-1)} disabled={value <= min}
          ...
          <div className="flex items-center px-2">
            <input type="number" min={min} max={effectiveMax * 5} step={step} value={typedValue}
            ...
            {unit && <span className="text-xs font-bold text-[#7A8C6E] ml-0.5 select-none">{unit}</span>}
          </div>
          <button type="button" onClick={() => handleStep(1)} aria-label="Increase value"
        ...
        <span className="text-xs font-semibold text-[#7A8C6E]">{effectiveMax.toLocaleString()} {unit}</span>
      </div>

      <div className="relative w-full py-1 flex items-center">
        <input type="range" min={min} max={effectiveMax} step={step} value={clampedValue}
          onChange={(e) => onChange(Number(e.target.value))}
          style={{ background: `linear-gradient(to right, #1A4D2E 0%, #2E7D52 ${pct}%, #EDE8DC ${pct}%, #EDE8DC 100%)` }}
      ...
      {presets && presets.length > 0 && (
        <div className="flex items-center flex-wrap gap-1.5 pt-0.5">
          <span className="text-[10px] uppercase font-bold text-[#7A8C6E] mr-1 tracking-wider">Presets:</span>
          {presets.map((preset, idx) => {
            ...
            return (
              <button key={idx} type="button" onClick={() => onChange(val)}
